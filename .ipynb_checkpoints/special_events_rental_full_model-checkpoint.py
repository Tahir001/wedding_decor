...
TABLECLOTH_DIR = os.path.join(ROOT_DIR, "table_cloths")
OUTPUT_DIR = os.path.join(ROOT_DIR, "output/tablecloths")

# IMAGES
BASE_IMAGE_PATH = os.path.join(ROOT_DIR, "base_image_table.png")

# DIMENSIONS (Optional: upgrade to 1024x1024 for better quality)
FIXED_WIDTH = 1024  # ← Changed from 768
FIXED_HEIGHT = 1024  # ← Changed from 768
REF_SIZE = 512  # ← Changed from 384 (proportional scaling)

# MODEL CONFIG
MODEL_NAME = "Qwen/Qwen-Image-Edit-2511"
# LORA_REPO = "lightx2v/Qwen-Image-Edit-2511-Lightning"  # ← COMMENTED OUT
# LORA_WEIGHTS = "Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors"  # ← COMMENTED OUT

# INFERENCE SETTINGS - FULL QUALITY MODE
STEPS = 40  # ← Changed from 4 (10x more steps for quality)
TRUE_CFG = 4.0  # ← Changed from 1.0 (stronger prompt adherence)
GUIDANCE = 1.0  # ← Same
SEED = 42
...
def load_pipeline():
    print_banner("LOADING QWEN MODEL - FULL QUALITY MODE")
    ...
    # NO Lightning LoRA - using full quality model
    # pipeline.load_lora_weights(LORA_REPO, weight_name=LORA_WEIGHTS)  # ← COMMENTED OUT
    
    pipeline.set_progress_bar_config(disable=True)
    
    print("✅ Full Quality Model Loaded Successfully")
    return pipeline
        ...
        try:
            ...
            with torch.inference_mode():
                result = pipeline(
                    ...
                    prompt=prompt,
                    negative_prompt="wrinkles, creases, folds, distortion, blurry, low quality, changed chairs, altered furniture",  # ← Enhanced
                    num_inference_steps=STEPS,  # Now 40 instead of 4
                    true_cfg_scale=TRUE_CFG,  # Now 4.0 instead of 1.0
                    guidance_scale=GUIDANCE,
